...
public class StageActor : MonoBehaviour, IActorDestroyHandler
{
    ...
    public void FinishSpawn(StageActor spawner, string run=null, float? lifetime=null)
    {
        ...
        {
            foreach (var handler in gameObject.GetComponentsInChildren<IActorLifetimeHandler>())
            {
                handler.HandleLifetime(lifetime.Value);
            }
    ...
    public IEnumerator RotateCoroutine(float? angle, float? dur, float? speed=null)
    {
        if (angle != null)
        {
            ...
            {
                totalDur = Mathf.Abs(startAngle - angle.Value) / speed.Value;
            }
            while (time < dur)
            {
